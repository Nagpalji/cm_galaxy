...
const StatsWithLineChart = ({ icon, color, stats, statTitle, series, options, type, height, ...rest }) => {
  return (
    <Card {...rest} style={{backgroundColor:options?.backGround, Color:"white"}}>
      <CardHeader className='align-items-start pb-0'>
        <div>
          <h2 className='font-weight-bolder text-light'>{stats}</h2>
          <CardText>{statTitle}</CardText>
        </div>
        <Avatar className='avatar-stats m-0' color={`light-${color}`} icon={icon} />
      </CardHeader>
